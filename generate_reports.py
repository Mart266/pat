...
INSTRUMENT = {
    "description": "Aegis Patrol Pro CZ5001 portable appliance tester",
    "calibration": "Jul 2026",
}

# Optional. Printed in the report footer when set; left out entirely when blank.
COMPETENCY = "Competency training completed 8 November 2022"
...
class ReportDoc(BaseDocTemplate):
    ...
    def __init__(self, path, report_name, ctx, **kw):
        ...
        BaseDocTemplate.__init__(self, path, pagesize=A4,
                                 ...
                                 title="%s - %s" % (COMPANY["name"], report_name),
                                 author=COMPANY["name"], **kw)
    ...
    def _decorate(self, canv, doc):
        ...
        canv.drawString(MARGIN, h - MARGIN - 4 * mm, COMPANY["name"].upper())

        canv.setFont("Helvetica", 7.5)
        canv.setFillColor(MUTED)
        canv.drawString(MARGIN, h - MARGIN - 9 * mm,
                        "Safety \u00b7 Compliance \u00b7 Peace of mind")

        canv.drawRightString(w - MARGIN, h - MARGIN - 3 * mm, COMPANY["phone"])
        canv.drawRightString(w - MARGIN, h - MARGIN - 6.5 * mm, COMPANY["web"])
        canv.drawRightString(w - MARGIN, h - MARGIN - 10 * mm, COMPANY["address"])
        canv.drawRightString(w - MARGIN, h - MARGIN - 13.5 * mm, "ABN " + COMPANY["abn"])

        canv.setStrokeColor(LINE)
        canv.setLineWidth(0.6)
        canv.line(MARGIN, h - MARGIN - 16 * mm, w - MARGIN, h - MARGIN - 16 * mm)
...
def instrument_note(st):
    text = ("Testing carried out with %s. Instrument calibration: %s. "
            "Tests performed to the requirements of %s."
            % (INSTRUMENT["description"], INSTRUMENT["calibration"], STANDARD_LONG))
    if COMPETENCY:
        text += (" Testing performed by a competent person as defined in %s: %s."
                 % (STANDARD, COMPETENCY))
...
def certificate(path, rows, ctx):
    ...
    story.append(Paragraph(
        ...
        % (COMPANY["name"], COMPANY["name"]), st["body"]))
